...
6. Direct Sierra signature typing now supports `felt252`, `bool`, `u128`, `u8/u16/u32/u64`, and `i8/i16/i32/i64/i128`; integer equality lowering (`*_eq`) is implemented for those families.
...
10. Compile and validate integer-signature Sierra contracts (including `u64` and signed widths) via `scripts/test/sierra_scalar_e2e.sh`.
...
4. Direct Sierra subset currently enforces:
   ...
   2. user signature types include `felt252`, `bool`, `u128`, `u8/u16/u32/u64`, and `i8/i16/i32/i64/i128`
   3. supported expression families center on literals, vars/let, `felt252` arithmetic, typed top-level equality (`felt252`, integer families), and wrapping `u128 add/sub/mul` with explicit `RangeCheck` threading
...
1. [`roadmap/inventory/sierra-coverage-matrix.json`](roadmap/inventory/sierra-coverage-matrix.json)
2. [`roadmap/inventory/sierra-coverage-summary.md`](roadmap/inventory/sierra-coverage-summary.md)

Current status (from the generated matrix):

1. Generic type IDs: `21 implemented`, `47 fail_fast`, `0 unresolved`
2. Generic libfunc IDs: `27 implemented`, `252 fail_fast`, `0 unresolved`
3. Extension modules: `4 implemented`, `58 fail_fast`, `0 unresolved`
