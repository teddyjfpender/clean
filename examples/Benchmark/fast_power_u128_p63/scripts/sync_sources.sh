...
GENERATED_SRC="$ROOT_DIR/examples/Cairo/fast_power_u128_p63/src/lib.cairo"
BASELINE_SRC="$ROOT_DIR/examples/Cairo-Baseline/fast_power_u128_p63/src/lib.cairo"
BASELINE_FAST_POWER_SRC="$ROOT_DIR/examples/Cairo-Baseline/fast_power_u128_p63/src/fast_power.cairo"
...
fn_match = re.search(r"fn\s+pow63_u128\s*\((?P<params>.*?)\)\s*->\s*(?P<ret>[A-Za-z0-9_]+)\s*\{", impl_block, re.S)
if not fn_match:
    fail("failed to locate function pow63_u128 in generated impl")
...
sig_match = re.match(r"fn\s+pow63_u128\s*\((?P<params>.*)\)\s*->\s*(?P<ret>[A-Za-z0-9_]+)", signature, re.S)
if not sig_match:
...
out = (
    ...
    f"pub fn pow63_generated({rendered_params}) -> {ret_ty} {body}\n"
)
