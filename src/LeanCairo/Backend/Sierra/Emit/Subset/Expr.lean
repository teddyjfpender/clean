...
partial def emitU16OverflowingWrapping
    (fnName : String)
    (env : Env)
    (lhs rhs : IRExpr .u16)
    (genericId : String)
  ...
  let mergedValue <- freshVarId fnName s!"{opTag}_result_merged"
  emitStoreTempTo .rangeCheck rcNonOverflow mergedRangeCheck
  emitStoreTempTo .u16 valueNonOverflow mergedValue
  emitJump (invocationIdx + 8)

  emitBranchAlign
  emitStoreTempTo .rangeCheck rcOverflow mergedRangeCheck
  emitStoreTempTo .u16 valueOverflow mergedValue
  setRangeCheckVar mergedRangeCheck
  pure (envAfterRhs, mergedValue)

partial def emitU16MulWrapping
    (fnName : String)
    (env : Env)
    (lhs rhs : IRExpr .u16) : EmitM (Env × Json) := do
  let (envAfterLhs, lhsVar) <- emitExpr fnName env lhs
  let (envAfterRhs, rhsVar) <- emitExpr fnName envAfterLhs rhs
  let rcIn <- requireRangeCheckVar fnName
  let _ <- registerTypeDecl .rangeCheck
  let _ <- registerTypeDecl .u16
  let _ <- registerTypeDecl .u32
  let _ <- registerTypeDecl (.nonZero "u32")

  let wideMulLibfuncId <- registerLibfuncDecl "u16_wide_mul" "u16_wide_mul" []
  let wideProductRaw <- freshVarId fnName "u16_mul_wide_product_raw"
  pushStmt (invocationStmtJson wideMulLibfuncId [lhsVar, rhsVar] [wideProductRaw])
  let wideProduct <- emitStoreTemp fnName .u32 wideProductRaw

  let modulusNonZero <- emitNonZeroU32Const fnName (2 ^ 16)

  let safeDivmodLibfuncId <- registerLibfuncDecl "u32_safe_divmod" "u32_safe_divmod" []
  let rcAfterDiv <- freshVarId fnName "u16_mul_range_check_after_divmod"
  let quotientRaw <- freshVarId fnName "u16_mul_quotient_raw"
  let remainderRaw <- freshVarId fnName "u16_mul_remainder_raw"
  pushStmt <|
    invocationStmtJson
      safeDivmodLibfuncId
      [rcIn, wideProduct, modulusNonZero]
      [rcAfterDiv, quotientRaw, remainderRaw]
  emitDrop fnName .u32 quotientRaw
  ...
  let downcastLibfuncId <-
    registerLibfuncDecl
      "downcast_u32_to_u16"
      "downcast"
      [typeArgJson u32TyId, typeArgJson u16TyId]

  let downcastInvocationIdx <- nextAbsoluteStatementIdx
  let downcastOverflowTarget := downcastInvocationIdx + 5

  let rcDowncastOk <- freshVarId fnName "u16_mul_range_check_downcast_ok"
  let valueDowncastOk <- freshVarId fnName "u16_mul_value_downcast_ok"
  let rcDowncastOverflow <- freshVarId fnName "u16_mul_range_check_downcast_overflow"
  pushStmt <|
    invocationStmtBranchesJson
      downcastLibfuncId
      [rcAfterDiv, remainderRaw]
  ...
  emitBranchAlign
  let mergedRangeCheck <- freshVarId fnName "u16_mul_range_check_merged"
  let mergedValue <- freshVarId fnName "u16_mul_value_merged"
  emitStoreTempTo .rangeCheck rcDowncastOk mergedRangeCheck
  emitStoreTempTo .u16 valueDowncastOk mergedValue
  emitJump (downcastInvocationIdx + 10)

  emitBranchAlign
  -- `remainderRaw` is mathematically `< 2^16`; overflow branch is unreachable.
  -- Keep branch shape total by materializing a deterministic placeholder.
  let unreachableFallback <- emitU16Const fnName 0
  emitStoreTempTo .rangeCheck rcDowncastOverflow mergedRangeCheck
  emitStoreTempTo .u16 unreachableFallback mergedValue
  setRangeCheckVar mergedRangeCheck
  pure (envAfterRhs, mergedValue)

partial def emitU8OverflowingWrapping
    (fnName : String)
    (env : Env)
    (lhs rhs : IRExpr .u8)
    (genericId : String)
  ...
  let mergedValue <- freshVarId fnName s!"{opTag}_result_merged"
  emitStoreTempTo .rangeCheck rcNonOverflow mergedRangeCheck
  emitStoreTempTo .u8 valueNonOverflow mergedValue
  emitJump (invocationIdx + 8)

  emitBranchAlign
  emitStoreTempTo .rangeCheck rcOverflow mergedRangeCheck
  emitStoreTempTo .u8 valueOverflow mergedValue
  setRangeCheckVar mergedRangeCheck
  pure (envAfterRhs, mergedValue)

partial def emitU8MulWrapping
    (fnName : String)
    (env : Env)
    (lhs rhs : IRExpr .u8) : EmitM (Env × Json) := do
  let (envAfterLhs, lhsVar) <- emitExpr fnName env lhs
  let (envAfterRhs, rhsVar) <- emitExpr fnName envAfterLhs rhs
  let rcIn <- requireRangeCheckVar fnName
  let _ <- registerTypeDecl .rangeCheck
  let _ <- registerTypeDecl .u8
  let _ <- registerTypeDecl .u16
  let _ <- registerTypeDecl (.nonZero "u16")

  let wideMulLibfuncId <- registerLibfuncDecl "u8_wide_mul" "u8_wide_mul" []
  let wideProductRaw <- freshVarId fnName "u8_mul_wide_product_raw"
  pushStmt (invocationStmtJson wideMulLibfuncId [lhsVar, rhsVar] [wideProductRaw])
  let wideProduct <- emitStoreTemp fnName .u16 wideProductRaw

  let modulusNonZero <- emitNonZeroU16Const fnName (2 ^ 8)

  let safeDivmodLibfuncId <- registerLibfuncDecl "u16_safe_divmod" "u16_safe_divmod" []
  let rcAfterDiv <- freshVarId fnName "u8_mul_range_check_after_divmod"
  let quotientRaw <- freshVarId fnName "u8_mul_quotient_raw"
  let remainderRaw <- freshVarId fnName "u8_mul_remainder_raw"
  pushStmt <|
    invocationStmtJson
      safeDivmodLibfuncId
      [rcIn, wideProduct, modulusNonZero]
      [rcAfterDiv, quotientRaw, remainderRaw]
  emitDrop fnName .u16 quotientRaw

  let u16TyId <- registerTypeDecl .u16
  let u8TyId <- registerTypeDecl .u8
  let downcastLibfuncId <-
    registerLibfuncDecl
      "downcast_u16_to_u8"
      "downcast"
      [typeArgJson u16TyId, typeArgJson u8TyId]

  let downcastInvocationIdx <- nextAbsoluteStatementIdx
  let downcastOverflowTarget := downcastInvocationIdx + 5

  let rcDowncastOk <- freshVarId fnName "u8_mul_range_check_downcast_ok"
  let valueDowncastOk <- freshVarId fnName "u8_mul_value_downcast_ok"
  let rcDowncastOverflow <- freshVarId fnName "u8_mul_range_check_downcast_overflow"
  pushStmt <|
    invocationStmtBranchesJson
      downcastLibfuncId
      [rcAfterDiv, remainderRaw]
  ...
  emitBranchAlign
  let mergedRangeCheck <- freshVarId fnName "u8_mul_range_check_merged"
  let mergedValue <- freshVarId fnName "u8_mul_value_merged"
  emitStoreTempTo .rangeCheck rcDowncastOk mergedRangeCheck
  emitStoreTempTo .u8 valueDowncastOk mergedValue
  emitJump (downcastInvocationIdx + 10)

  emitBranchAlign
  -- `remainderRaw` is mathematically `< 2^8`; overflow branch is unreachable.
  -- Keep branch shape total by materializing a deterministic placeholder.
  let unreachableFallback <- emitU8Const fnName 0
  emitStoreTempTo .rangeCheck rcDowncastOverflow mergedRangeCheck
  emitStoreTempTo .u8 unreachableFallback mergedValue
  setRangeCheckVar mergedRangeCheck
  pure (envAfterRhs, mergedValue)

partial def emitI16OverflowingWrapping
    (fnName : String)
    (env : Env)
    (lhs rhs : IRExpr .i16)
    (genericId : String)
  ...
  let mergedValue <- freshVarId fnName s!"{opTag}_result_merged"
  emitStoreTempTo .rangeCheck rcNonOverflow mergedRangeCheck
  emitStoreTempTo .i16 valueNonOverflow mergedValue
  emitJump (invocationIdx + 8)

  emitBranchAlign
  emitStoreTempTo .rangeCheck rcOverflow mergedRangeCheck
  emitStoreTempTo .i16 valueOverflow mergedValue
  setRangeCheckVar mergedRangeCheck
  pure (envAfterRhs, mergedValue)

partial def emitI16MulChecked
    (fnName : String)
    (env : Env)
    (lhs rhs : IRExpr .i16) : EmitM (Env × Json) := do
  let (envAfterLhs, lhsVar) <- emitExpr fnName env lhs
  let (envAfterRhs, rhsVar) <- emitExpr fnName envAfterLhs rhs
  let rcIn <- requireRangeCheckVar fnName
  let _ <- registerTypeDecl .rangeCheck
  let _ <- registerTypeDecl .i16
  let _ <- registerTypeDecl .i32

  let wideMulLibfuncId <- registerLibfuncDecl "i16_wide_mul" "i16_wide_mul" []
  let wideProductRaw <- freshVarId fnName "i16_mul_wide_product_raw"
  pushStmt (invocationStmtJson wideMulLibfuncId [lhsVar, rhsVar] [wideProductRaw])
  let wideProduct <- emitStoreTemp fnName .i32 wideProductRaw

  let i32TyId <- registerTypeDecl .i32
  let i16TyId <- registerTypeDecl .i16
  let downcastLibfuncId <-
    registerLibfuncDecl
      "downcast_i32_to_i16"
      "downcast"
      [typeArgJson i32TyId, typeArgJson i16TyId]

  let downcastInvocationIdx <- nextAbsoluteStatementIdx
  let downcastOverflowTarget := downcastInvocationIdx + 5

  let rcDowncastOk <- freshVarId fnName "i16_mul_range_check_downcast_ok"
  let valueDowncastOk <- freshVarId fnName "i16_mul_value_downcast_ok"
  let rcDowncastOverflow <- freshVarId fnName "i16_mul_range_check_downcast_overflow"
  pushStmt <|
    invocationStmtBranchesJson
      downcastLibfuncId
      [rcIn, wideProduct]
  ...
  emitBranchAlign
  let mergedRangeCheck <- freshVarId fnName "i16_mul_range_check_merged"
  let mergedValue <- freshVarId fnName "i16_mul_value_merged"
  emitStoreTempTo .rangeCheck rcDowncastOk mergedRangeCheck
  emitStoreTempTo .i16 valueDowncastOk mergedValue
  emitJump (downcastInvocationIdx + 10)

  emitBranchAlign
  -- Checked i16 lane: downcast overflow indicates caller precondition violation.
  -- Keep branch shape total by materializing a deterministic placeholder.
  let unreachableFallback <- emitI16Const fnName 0
  emitStoreTempTo .rangeCheck rcDowncastOverflow mergedRangeCheck
  emitStoreTempTo .i16 unreachableFallback mergedValue
  setRangeCheckVar mergedRangeCheck
  pure (envAfterRhs, mergedValue)
...
partial def emitExpr (fnName : String) (env : Env) : IRExpr ty -> EmitM (Env × Json)
  ...
  | .litInt .u32 value => do
      ...
      pure (env, outVar)
  | .litInt .u16 value => do
      let outVar <- emitU16Const fnName value
      pure (env, outVar)
  | .litInt .u8 value => do
      let outVar <- emitU8Const fnName value
      pure (env, outVar)
  | .litInt .i16 value => do
      let outVar <- emitI16Const fnName value
      pure (env, outVar)
  | .litInt ty _ =>
  ...
  | .addInt .u16 lhs rhs =>
      emitU16OverflowingWrapping fnName env lhs rhs "u16_overflowing_add" "u16_add"
  | .addInt .u8 lhs rhs =>
      emitU8OverflowingWrapping fnName env lhs rhs "u8_overflowing_add" "u8_add"
  | .addInt .i16 lhs rhs =>
      emitI16OverflowingWrapping fnName env lhs rhs "i16_overflowing_add_impl" "i16_add"
  | .addInt ty _ _ =>
  ...
  | .subInt .u16 lhs rhs =>
      emitU16OverflowingWrapping fnName env lhs rhs "u16_overflowing_sub" "u16_sub"
  | .subInt .u8 lhs rhs =>
      emitU8OverflowingWrapping fnName env lhs rhs "u8_overflowing_sub" "u8_sub"
  | .subInt .i16 lhs rhs =>
      emitI16OverflowingWrapping fnName env lhs rhs "i16_overflowing_sub_impl" "i16_sub"
  | .subInt ty _ _ =>
  ...
  | .mulInt .u16 lhs rhs =>
      emitU16MulWrapping fnName env lhs rhs
  | .mulInt .u8 lhs rhs =>
      emitU8MulWrapping fnName env lhs rhs
  | .mulInt .i16 lhs rhs =>
      emitI16MulChecked fnName env lhs rhs
  | .mulInt ty _ _ =>
